...
assert all([i[-6:-4] == j[-7:-5] for i,j in zip(image_filenames, json_filenames)])
...
ENVIRONMENT= {'darknessLevel': 0, 'darknessLock': False, 'globalLight': {'enabled': True, 'alpha': 0.5, 'bright': False, 'color': None, 'coloration': 1, 'luminosity': 0, 'saturation': 0, 'contrast': 0, 'shadows': 0, 'darkness': {'min': 0, 'max': 0.5}}, 'cycle': True, 'base': {'hue': 0, 'intensity': 0, 'luminosity': 0, 'saturation': 0, 'shadows': 0}, 'dark': {'hue': 0.7138888888888889, 'intensity': 0, 'luminosity': -0.25, 'saturation': 0, 'shadows': 0}}

def convert_map(dirpath, basename, tile_path, json_filenames, image_filenames, ground_floor_index, floor_height):
    output_json = None
    previous_floor_polygons = None
    total_floors = len(json_filenames)
    print(json_filenames)
    print(image_filenames)
    for current_floor_index, (jsonfile, imagefile) in enumerate(zip(json_filenames, image_filenames)):
        ...
        output_json = update_json(output_json, dirpath, basename, tile_path, current_floor_index, imagefile, jsonfile, ground_floor_index, total_floors, floor_height, current_floor_polygons)
        ...
        #    print(poly)
        mask_polygons = previous_floor_polygons + current_floor_polygons if previous_floor_polygons is not None else current_floor_polygons
        create_image(dirpath, imagefile, mask_polygons, get_below_or_ground(ground_floor_index, current_floor_index))
...
def update_json(output_json, dirpath, basename, tile_path, current_floor_index, imagefile, jsonfile, ground_floor_index, total_floors, floor_height, current_floor_polygons):
    ...
    if output_json is None:
        ...
        output_json['flags'] = create_flags(total_floors, ground_floor_index, floor_height)
    ...
    #for wall in current_json['walls']:
        #print(wall)
    new_walls =  [ wall | { 'flags': {'wall-height': {'bottom': bottom_height, 'top': top_height}}} for wall in current_json['walls']]
    ...
    #    print(p)
    new_region = {
                'name': f'Inside Buildings {bottom_height} - {top_height}',
                'color': "#"+''.join([random.choice('0123456789abcdef') for j in range(6)]),
                'shapes': [{'type': 'polygon', 'points': [i for j in polygon for i in j]} for polygon in current_floor_polygons],
                'hole': False,
                'elevation': {'bottom': bottom_height, 'top': top_height - 1},
                "behaviors": [
                    {
    ...
    output_json['regions'] = [new_region] if 'regions' not in output_json else output_json['regions'] + [new_region]
    ...
    output_json['tiles'] = [new_tile] if 'tiles' not in output_json else output_json['tiles'] + [new_tile]
...
def create_grid(initial_json):
    return {'type': 1, 'size': initial_json['grid'], 'style': 'solidLines', 'thickness': 1, 'color': '#000000', 'alpha': 0.5, 'distance': initial_json['gridDistance'], 'units': initial_json['gridUnits']}



def create_flags(number_of_levels, zero_floor, floor_height):
    return {'levels': {'lightMasking': False, 'sceneLevels': [[floor_height*i, floor_height*(i+1), f'level {i}'] for i in range(-zero_floor, number_of_levels - zero_floor)], 'wall-height': {'advancedVision': True}}}
...
def create_floor_polygons(dirpath, jsonfile):
    ...
    walls = [((wall['c'][0], wall['c'][1]), (wall['c'][2], wall['c'][3])) for wall in map_json['walls']]
    ...
    points_cycles = [[pointdict.inverse[point] for point in cycle] for cycle in cycles if len(cycle) > 2]
    points_cycles = [convert_node_to_edge_poly(cycle) for cycle in points_cycles]
...
def all_points_inside(test_cycle, boundary_cycle):
    ...
    return all([is_inside(boundary_cycle, point[0][0], point[0][1]) and is_inside(boundary_cycle, point[1][0], point[1][1]) for point in test_cycle])
...
def is_on_edge(edge0, edge1, xp, yp):
    if (edge0[0] == xp and edge0[1] == yp) or (edge1[0] == xp and edge1[1] == yp):
        return True
...
convert_map(dirpath, basepath, args.tile_save_path, json_filenames, image_filenames, args.ground_floor, args.floor_height)
